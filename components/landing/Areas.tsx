      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {areas.map((area) => (
            <div
              key={area.number}
              className={`area-card bg-white rounded-3xl border border-white p-8 shadow-sm flex flex-col`}
            >
              ...
              <div className="flex flex-wrap gap-2 mt-auto">
                {area.skills.map((skill) => (
                  <span
                    key={skill}
                    className="skill-tag bg-gray-200 text-gray-900 px-3 py-1 rounded-full text-xs font-medium"
                  >
                    {skill}
                  </span>
                ))}
