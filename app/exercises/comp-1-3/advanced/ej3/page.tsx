...
import { AlertCircle, AlertTriangle, CheckCircle, Shield, X } from 'lucide-react'
import { useToast } from '@/hooks/use-toast'
import { compareNumbers, compareText } from '@/utils/answer-validators'

export default function EjercicioComp13Avanzado3() {
  const { toast } = useToast()
  const [datasetUrl, setDatasetUrl] = useState<string | null>(null)
  useEffect(() => {
    setDatasetUrl('/api/datasets/comp-1-3/advanced/ej3/default')
  }, [])
  ...
  const [c1, setC1] = useState('') // país mayor producción total (Arábica+Robusta)
  const [c2, setC2] = useState('') // producción Robusta en Colombia (t)
  const [c3, setC3] = useState('') // diferencia Arábica - Robusta en Honduras (t)

  // No conocemos las respuestas desde el documento; dejarlas como TODO y permitir ajustar rápido:
  const EXPECTED = {
    paisMayor: 'Brasil',
    robustaColombia: 0,
    diferenciaHonduras: 0,
  }

  const v1 = c1 ? compareText(c1, EXPECTED.paisMayor).ok : null
  const v2 = c2 ? compareNumbers(c2, EXPECTED.robustaColombia).ok : null
  const v3 = c3 ? compareNumbers(c3, EXPECTED.diferenciaHonduras).ok : null
          ...
          <CardContent className="p-6">
            ...
            <div className="mb-6">
              <div className="bg-gray-50 p-4 rounded-xl border-l-4 border-purple-500">
                <p className="text-gray-700">Genera un gráfico dinámico (por país y tipo de café) y responde:</p>
              </div>
            ...
            <Field label="Casilla 1: ¿Cuál es el país con mayor producción total de café?" placeholder="Ej: Brasil" value={c1} onChange={setC1} state={v1} />
            <Field label="Casilla 2: ¿Cuál es la producción total (ton) del café Robusta en Colombia?" placeholder="Ej: 0" value={c2} onChange={setC2} state={v2} />
            <Field label="Casilla 3: ¿Cuál es la diferencia (ton) entre Arábica y Robusta en Honduras?" placeholder="Ej: 0" value={c3} onChange={setC3} state={v3} />
