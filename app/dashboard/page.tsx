...
export default function Dashboard() {
  ...
  const filteredCompetences = competences.filter((competence) => {
    const matchesSearch = competence.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                        competence.description.toLowerCase().includes(searchTerm.toLowerCase())
    
    const hasQuestions = counts[competence.id] >= 3
      ...
      <main className="flex-1 lg:ml-64 px-4 lg:px-8 py-4 lg:py-8">
        <div className="max-w-7xl mx-auto">
                ...
                <div className="flex gap-2 lg:gap-3">
                  {["Todas", "Iniciadas", "Pendientes"].map((filter) => (
                    <Button
                      ...
                      className={`
                        px-3 lg:px-4 py-2 lg:py-2.5 text-xs lg:text-sm font-medium rounded-xl lg:rounded-2xl transition-all duration-200
                        ${filterType === filter 
          ...
          <div className="space-y-8 lg:space-y-12">
            {Object.entries(groupedCompetences).map(([dimension, competences]) => (
